...
public class BlockHuntGame extends Recipes {
	...
	public void isGameOver(Teams roundWinner) {
			...
			for (UUID uuid : arena.getPlayers()) {
				...
				if (arena.getTeam(player).equals(Teams.BLUE)) {
					GameCoreMain.getInstance().sendTitle(player, "&6VICTORY",
							"&7There are no players left on the other team!", 0, 9, 1);
		...
		if (redWins.size() == 2) {
			...
			for (UUID uuid : arena.getPlayers()) {
				...
				if (arena.getTeam(player).equals(Teams.RED)) {
					GameCoreMain.getInstance().sendTitle(player, "&6VICTORY",
							"&7Your team won " + redWins.size() + " &7out of 3 rounds!", 0, 9, 1);
					SongPlayer.playSong(player, Songs.WIN);
					giveTrophy(player);
				} else {
					GameCoreMain.getInstance().sendTitle(player, "&cGAME OVER", "&7You didn't win this time!", 0, 5, 1);
		...
		if (blueWins.size() == 2) {
			...
			for (UUID uuid : arena.getPlayers()) {
				...
				if (arena.getTeam(player).equals(Teams.BLUE)) {
					GameCoreMain.getInstance().sendTitle(player, "&6VICTORY",
							"&7Your team won " + blueWins.size() + " &7out of 3 rounds!", 0, 9, 1);
					SongPlayer.playSong(player, Songs.WIN);
					giveTrophy(player);
				} else {
					GameCoreMain.getInstance().sendTitle(player, "&cGAME OVER", "&7You didn't win this time!", 0, 5, 1);
			...
			if (redWins.size() > blueWins.size()) {
				...
				for (UUID uuid : arena.getPlayers()) {
					...
					if (arena.getTeam(player).equals(Teams.RED)) {
						...
						GameCoreMain.getInstance().sendTitle(player, "&cGAME OVER", "&7You didn't win this time!", 0, 5,
								1);
						SongPlayer.playSong(player, Songs.LOSE);
					}
				}
		...
		// if (finishedTeams.size() == 2 && preGame) {
		// arena.sendMessage(ChatColor.GREEN
		// + "Time is up! Both Teams finished, so you'll be assigned a new block of a
		// higher difficulty. Your inventory will be cleared and you'll be teleported
		// back to spawn.");
		// if (getBlock(Teams.RED).getDifficulty() == 5) {
		// arena.sendMessage(
		// ChatColor.translateAlternateColorCodes('&', "&e&lDRAW! You're both too good.
		// Have a trophy!"));
		// finishedTeams.clear();
		// for (UUID uuid : arena.getPlayers()) {
		// Player player = Bukkit.getPlayer(uuid);
		...
		// player.sendMessage(
		// // ChatColor.GREEN + "Your new block is " +
		...
		// crafterMeta.setDisplayName(ChatColor.GREEN + "Crafting Table" +
		...
		// World w = arena.getSpawn().getWorld();
		// int redy = w.getHighestBlockYAt(redx, redz) + 1;
		// Location redLoc = new Location(w, redx, redy, redz);

		// Random b = new Random();
		// int bluex = b.nextInt(xMax - xMin + 1) + xMin;
		// int bluez = b.nextInt(zMax - zMin + 1) + zMin;
		// // World w2 = arena.getSpawn().getWorld();
		// int bluey = w.getHighestBlockYAt(bluex, bluez) + 1;
		// Location blueLoc = new Location(w2, bluex, bluey, bluez);
		//
		// for (UUID uuid : arena.getPlayers()) {
		// Player player = Bukkit.getPlayer(uuid);
		...
		// "&a&lRANDOM! &eYour team has been teleported to: ") + ChatColor.GOLD + redx +
		// ", " + redy
		// + ", " + redz);
		// if (hasModifier(ModifiedTypes.MOB_DAMAGE) ||
		// hasModifier(ModifiedTypes.FALL_DAMAGE)) {
		// player.sendMessage(ChatColor.RED
		// + "If you die, your inventory will be kept, but you'll be sent back to the
		// original spawn.");
		//
		// }
		//
		...
		// player.sendMessage(ChatColor.translateAlternateColorCodes('&',
		// "&a&lRANDOM! &eYour team has been teleported to: ") + ChatColor.GOLD + bluex
		// + ", " + bluey
		// + ", " + bluez);
		// player.sendMessage(ChatColor.RED + "The terrain may be generating... Please
		// be patient...");
		// if (hasModifier(ModifiedTypes.MOB_DAMAGE) ||
		// hasModifier(ModifiedTypes.FALL_DAMAGE)) {
		// player.sendMessage(ChatColor.RED
	...
	public void startBlockHunt(GameType type) {
		...
		// arena.sendMessage(ChatColor.GOLD + "[DEBUG] Initial Block1 Difficulty: " +
		// block1.getDifficulty());
		Blocks block2 = randomBlock();
		// arena.sendMessage(ChatColor.GOLD + "[DEBUG] Initial Block2 Difficulty: " +
		// block2.getDifficulty());
		do {
			block2 = randomBlock();
			// arena.sendMessage(ChatColor.GOLD + "[DEBUG] New Block2 Difficulty: " +
			// block2.getDifficulty());
						...
						if (!finishedTeams.contains(arena.getTeam(player))) {
							...
							if (player.getLocation().getBlock().getRelative(BlockFace.DOWN).getType()
								...
								if (finishedTeams.size() == 2) {
									...
									arena.sendMessage(ChatColor.RED
											+ "FATAL ERROR! STOPPING GAME BEFORE EVERYTHING BREAKS... (ERR CODE: TWO_FINISHED_TEAMS)");
									arena.reset();
									removeRecipies();
									runnable.cancel();
